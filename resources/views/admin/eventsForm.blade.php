        ...
        <div class="card-body">
            <div class="col-md-6 offset-md-3">
                <form class="container" method="POST"
                      action="{{ ($event->id == 0) ? route('events.store') : route('events.update', ['slug' => $event->slug]) }}">
                    @csrf
                    @if($event->id != 0)
                        @method("PUT")
                    @endif
                    ...
                    <div class="form-group row">
                        <label for="hashtag" class="col-md-4 col-form-label text-md-right">{{ __('events.hashtag') }}
                            :</label>
                        <div class="col-md-6 text-center">
                            <input id="hashtag" type="text" class="form-control" name="hashtag"
                                   value="{{$event->hashtag}}" required autofocus>
                        </div>
                    </div>
                    <div class="form-group row">
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-md-6 text-center">
                            <input id="url" type="url" class="form-control" name="url" value="{{$event->url}}" required
                                   autofocus>
                        </div>
                    </div>
                    <div class="row">
                        <div class="form-group col-sm-4 offset-sm-2 col-6">
                            ...
                            <input id="begin" type="date" class="form-control" name="begin"
                                   value="{{ (new Carbon\Carbon($event->begin))->format("Y-m-d") }}" required autofocus>
                        </div>
                        <div class="form-group col-sm-4 offset-sm-2 col-6">
                            <label for="end">{{ __('events.end') }}:</label>
                            <input id="end" type="date" class="form-control" name="end"
                                   value="{{ (new Carbon\Carbon($event->end))->format("Y-m-d") }}" required autofocus>
                        </div>
                    </div>
                    <div class="form-group row">
                        <label for="nearest_station_name"
                               class="col-md-4 col-form-label text-md-right">{{ __('events.closestStation') }}:</label>
                        <div class="col-md-6 text-left" id="autocomplete-form">
                            <input type="text" id="station-autocomplete" name="nearest_station_name"
                                   ...
                                   value="{{ $event->getTrainStation()->name }}" required>
                        </div>
                    </div>
                    <div class="form-group text-center">
                        ...
                        @if($event->id != 0)
                            <a href="#" class="btn btn-danger" role="button" data-mdb-toggle="modal"
                               data-mdb-target="#delete-modal-{{ $event->id }}">
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-footer">
                        <button type="button" class="btn btn-light"
                                data-mdb-dismiss="modal">{{__('menu.abort')}}</button>
                        <a href="{{ URL::signedRoute('events.delete', ['slug' => $event->slug]) }}"
                           class="btn btn-danger" id="modal-delete">{{__('modals.delete-confirm')}}</a>
                    </div>
                </div><!-- /.modal-content -->
            </div><!-- /.modal-dialog -->
        </div><!-- /.modal -->
