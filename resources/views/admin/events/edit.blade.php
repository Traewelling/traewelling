        ...
        <div class="col-md-4">
            <div class="card mb-2">
                <div class="card-body">
                    ...
                    @foreach(\App\Enum\EventCategory::cases() as $eventCategory)
                        <input type="checkbox"
                               id="category-{{$eventCategory->value}}"
                               value="{{$eventCategory->value}}"
                               onchange="handleCategoryInput(this)"
                            {{$event->categories->filter(fn($categoryModel) => $categoryModel->category->value === $eventCategory->value)->count() ? 'checked' : ''}}
                        />
                        <label for="category-{{$eventCategory->value}}">
                            {{__('events.category.'.$eventCategory->value)}}
                        </label>
                        <br>
                    @endforeach
                    ...
                    <script>
                        ...
                        function handleCategoryInput(input) {
                            if (input.checked) {
                                TrwlEvent.addCategory({{$event->id}}, input.value);
                            } else {
                                TrwlEvent.deleteCategory({{$event->id}}, input.value);
                            }
                        }
                    </script>
                </div>
