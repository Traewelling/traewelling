        ...
        <ul class="timeline">
            <li>
                ...
                <span class="text-trwl float-right">
                    {{ date('H:i', strtotime($status->trainCheckin->departure)) }}
                </span>
                ...
                <p class="train-status text-muted">
                    ...
                    <span class="pl-2"><i class="fa fa-route d-inline"></i>&nbsp;
                        {{number($status->trainCheckin->distance, 0)}}<small>km</small>
                    </span>
                    @php($dur = secondsToDuration(strtotime($status->trainCheckin->arrival)
- strtotime($status->trainCheckin->departure)))
                ...
                ?>
                    <p class="text-muted font-italic">
                        {{ __('stationboard.next-stop') }}: {!! stationLink($stops[$nextStopIndex]->stop->name) !!}
                    </p>
                @endif
            </li>
            <li>
    ...
    <ul class="list-inline">
        ...
        @if(Auth::check())
            ...
            @if($currentUser->id == $status->user_id && $status->likes->count() !== 0)
                d-none
            @endif
                list-inline-item d-lg-none" id="avatar-small-{{ $status->id }}"
                data-selflike="{{ $currentUser->id == $status->user_id }}">
                    <a href="{{ route('account.show', ['username' => $status->user->username]) }}">
                        <img src="{{ route('account.showProfilePicture', ['username' => $status->user->username]) }}"
            ...
            <li class="list-inline-item like-text">
                <span class="like
{{ $status->likes->where('user_id', $currentUser->id)->first() === null ? 'far fa-star' : 'fas fa-star'}}"
                      data-statusid="{{ $status->id }}"></span>
                <span class="pl-1 @if($status->likes->count() == 0) d-none @endif" id="like-count-{{ $status->id }}">
                    {{ $status->likes->count() }}
                </span>
            </li>
            @if($currentUser->id == $status->user_id)
