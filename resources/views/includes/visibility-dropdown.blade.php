<div class="btn-group">
    <button class="btn btn-sm btn-outline-twitter dropdown-toggle"
            ...
            aria-expanded="false"
    >
        <i class="fa fa-{{['globe-americas', 'lock-open', 'user-friends', 'lock'][auth()->user()->default_status_visibility]}}"
           aria-hidden="true"></i>
    </button>
    <ul class="dropdown-menu" aria-labelledby="visibilityDropdownButton">
        @foreach(\App\Enum\StatusVisibility::getList() as $visibility)
            @if(auth()->user()->default_status_visibility <= $visibility)
    ...
    <input type="hidden" id="checkinVisibility" name="checkinVisibility"
           value="{{auth()->user()->default_status_visibility}}"/>
</div>
